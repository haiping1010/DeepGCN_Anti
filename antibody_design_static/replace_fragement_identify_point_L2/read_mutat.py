...
def caculate_mut(frag_ori,frag_rep,beg_num,dict_p_r):
     ...
     for i in  range(len(frag_ori)):
         if frag_ori[i] != frag_rep[i]:
               arr_n.append('L'+str(i+int(beg_num)+begin)+'_'+frag_rep[i])
               dict_p_r['L'+str(i+int(beg_num)+begin)].append(frag_rep[i])
...
for key, value in  dict_without_empty.items():
      ...
      print (value_n)
      value_n.append(resid_to_name[pos_T+'L'])
      dict_final[pos_T+'L'] = value_n
